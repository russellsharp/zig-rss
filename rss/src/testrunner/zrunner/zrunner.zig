...
fn runTests(
    ...
    // var total_timer: std.Io.Timer = try std.Io.Timer.start();
    // var test_timer: std.Io.Timer = try std.Io.Timer.start();
    var total_start = std.Io.Clock.now(.awake, io);
    ...
    report.total_duration = Duration.fromNanos(@intCast(total_start.untilNow(io, .awake).nanoseconds));
}
...
const Test = struct {
    ...
    fn run(
        ...
        // timer.reset();
        var test_start = std.Io.Clock.now(.awake, io);

        // Try to run the test:
        const result = self.test_fn.func();

        // const duration = Duration.fromNanos(timer.read());
        const duration = Duration.fromNanos(@intCast(test_start.untilNow(io, .awake).nanoseconds));
    ...
    fn isTestingZig(alloc: std.mem.Allocator, io: std.Io, debug_info: *std.debug.SelfInfo, address: usize) bool {
        const symbols: std.ArrayList(std.debug.Symbol) = .empty;
        var symbols_ptr: *std.ArrayList(std.debug.Symbol) = @constCast(&symbols);
        _ = &symbols_ptr;
        debug_info.getSymbols(io, alloc, alloc, address, true, symbols_ptr) catch return false;
